...
try {
  if (-not $Msix) {
    ...
    Invoke-Checked bare-build @('--name', 'hello-pear-bare', '--standalone', '--host', 'win32-x64', '--out', $out, 'bin.js')
  ...
  try {
    ...
    Invoke-Checked bare-build @('--manifest', './manifest/win32-msix.appxmanifest', '--name', 'hello-pear-bare', '--host', 'win32-x64', '--subject', $env:MSIX_CERT_SUBJECT, '--out', './out/win32-x64-msix-dir', 'bin.js')
